            ...
            <script>
                function scrollToTop() {
                    try {
                        ...
                        document.querySelectorAll('.main, [data-testid="stMain"]').forEach(el => {
    ...
    try:
        if section == "📊 Business Intelligence":
            if module == "💰 Retail Prediction":
                # Retail Prediction 분석 페이지들 (기존 페이지 연결)
                if page == "📊 전체 분석 요약":
                    show_retail_analysis_page()  # 기존 Online Retail 분석 페이지
                elif page == "1️⃣ 데이터 로딩 및 개요":
                    # 임시로 retail 페이지 사용 + 구현 예정 메시지
                    st.info("📋 1️⃣ Retail 데이터 로딩 & 개요 페이지 구현 예정")
                    show_retail_analysis_page()  
                elif page == "2️⃣ 데이터 정제 & 전처리":
                    st.info("🧹 2️⃣ 데이터 정제 & 전처리 페이지 구현 예정")
                    show_retail_analysis_page()
                elif page == "3️⃣ 특성공학 & 파생변수 생성":
                    st.info("⚙️ 3️⃣ 특성공학 & 파생변수 생성 페이지 구현 예정")
                    show_retail_analysis_page()
                elif page == "4️⃣ 타겟변수 생성":
                    st.info("🎯 4️⃣ 타겟변수 생성 페이지 구현 예정")
                    show_retail_analysis_page()
                elif page == "5️⃣ 선형회귀 모델링":
                    st.info("🤖 5️⃣ 선형회귀 모델링 페이지 구현 예정")
                    show_retail_analysis_page()
                elif page == "6️⃣ 모델 평가 & 해석":
                    st.info("📊 6️⃣ 모델 평가 & 해석 페이지 구현 예정")
                    show_retail_analysis_page()
                else:
                    show_retail_analysis_page()  # 기본값
                    
            elif module == "👥 Customer Segmentation": 
                # Customer Segmentation 분석 페이지들 (기존 페이지 연결)
                if page == "1️⃣ 데이터 로딩 및 개요":
                    show_data_overview_page()  # 기존 페이지 연결
                elif page == "2️⃣ 탐색적 데이터 분석":
                    show_exploratory_analysis_page()  # 기존 페이지 연결
                elif page == "3️⃣ 클러스터링 분석":
                    show_clustering_analysis_page()  # 기존 페이지 연결
                elif page == "4️⃣ 주성분 분석":
                    show_pca_analysis_page()  # 기존 페이지 연결
                elif page == "5️⃣ 오토인코더 딥러닝":
                    show_deep_learning_analysis_page()  # 기존 페이지 연겴
                elif page == "6️⃣ customer segmentation 예측":
                    show_customer_prediction_page()  # 기존 페이지 연겴
                elif page == "7️⃣ 마케팅 전략":
                    show_marketing_strategy_page()  # 기존 페이지 연겴
                else:
                    show_data_overview_page()  # 기본값
            ...
            if module == "🔒 네트워크 보안 이상 탐지":
                ...
                if show_security_analysis_page is not None:
                    # 모든 Security 단계는 기존 security 페이지로 연겴 (나중에 개별 구현 가능)
                    if page in ["1️⃣ 데이터 로딩 및 개요", "2️⃣ 탐색적 데이터 분석", 
                               "3️⃣ 공격 패턴 심화 분석", "4️⃣ 딥러닝 모델링",
                               "5️⃣ Overfitting 해결 검증", "6️⃣ 실시간 예측 테스트", 
                               "7️⃣ 종합 성능 평가"]:
                        # 현재 선택된 단계에 대한 정보 표시
                        st.info(f"🔍 Security Analytics: {page}")
                        st.markdown("""
                        **CICIDS2017 데이터섹을 사용한 네트워크 이상 탐지**
                        - 대용량 네트워크 트래픽 데이터 분석
                        - 하이브리드 딥러닝 모델 (MLP + CNN) 사용
                        - 실시간 보안 위협 탐지 시스템 구현
                        - 금융권 SI 보안 전문가 양성 목적
                        """)
                        show_security_analysis_page()  # 기존 보안 페이지 연겴
                    else:
                    ...
                    **개발 진행 상황:**
                    ...
                    - Phase 2: 계층형 네비게이션 완료 ✅  
                    - Phase 3: Security 모듈 재활성화 예정 🛠️
                    """)
        else:
            # 알 수 없는 섹션인 경우 기본 페이지 표시
...
def main():
    ...
    try:
        ...
        if selected_section and selected_module and selected_page:
            ...
            # 기본 페이지 표시 (첫 로드 또는 오류 시)
            st.info("🚀 시작하려면 좌쪽 메뉴에서 Business Intelligence > Retail Analytics를 선택하세요!")
            route_to_page(
